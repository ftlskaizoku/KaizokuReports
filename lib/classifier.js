...
function classifyCandle(c,prev,history=[]){
  const{open,high,low,close}=c,range=high-low,bodyHigh=Math.max(open,close),bodyLow=Math.min(open,close),body=bodyHigh-bodyLow,upperWick=high-bodyHigh,lowerWick=bodyLow-low;
  const bodyPct=range>0?(body/range)*100:0,upperWickPct=range>0?(upperWick/range)*100:0,lowerWickPct=range>0?(lowerWick/range)*100:0,closePos=range>0?(close-low)/range:0.5;
  const isBull=close>open,isBear=close<open,structure=detectStructure(history);
  let type='spinning_top';
  if(bodyPct<5)type='doji';
  else if(bodyPct<20&&upperWickPct>15&&lowerWickPct>15)type='spinning_top';
  else if(prev){const ctx=classifyContextual(c,prev);if(ctx)type=ctx;else type=basePattern({bodyPct,upperWickPct,lowerWickPct,closePos,isBull,isBear});}
  ...
  return{candle_type:type,structure_label:structure,body_pct:+bodyPct.toFixed(2),upper_wick_pct:+upperWickPct.toFixed(2),lower_wick_pct:+lowerWickPct.toFixed(2),close_position:+closePos.toFixed(4),range_points:+range.toFixed(4),body_points:+body.toFixed(4),d1_bias:bias,bias_confidence:confidence};
}
...
function classifyContextual(c,prev){
  if(!prev)return null;
  const ch=Math.max(c.open,c.close),cl=Math.min(c.open,c.close),ph=Math.max(prev.open,prev.close),pl=Math.min(prev.open,prev.close);
...
function detectStructure(history){
  ...
  const closes=history.map(c=>+c.close),half=Math.floor(closes.length/2);
  const avg1=closes.slice(0,half).reduce((a,b)=>a+b,0)/half,avg2=closes.slice(half).reduce((a,b)=>a+b,0)/(closes.length-half);
  const highs=history.map(c=>+c.high).slice(-5),lows=history.map(c=>+c.low).slice(-5);
  const hh=highs.filter((h,i)=>i>0&&h>highs[i-1]).length,ll=lows.filter((l,i)=>i>0&&l<lows[i-1]).length;
  if(avg2>avg1*1.005&&hh>=2)return'uptrend';
  if(avg2<avg1*0.995&&ll>=2)return'downtrend';
  return'ranging';
}
function deriveBias(type,structure){
  const bull=['strong_bullish','bullish_pin_bar','hammer','bullish_engulfing','moderate_bullish'];
  const bear=['strong_bearish','bearish_pin_bar','shooting_star','bearish_engulfing','moderate_bearish'];
  ...
  if(bull.includes(type)){bias='bullish';c+=b.bull;}
  else if(bear.includes(type)){bias='bearish';c+=b.bear;}
...
function classifyOutcome(prevBias,prevCandle,todayCandle){
  ...
  if(Math.abs(ret)>=0.25){outcomeType=dir===prevBias?'continuation':Math.abs(ret)>=0.5?'reversal':'pullback';}
