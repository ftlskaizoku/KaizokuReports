...
function init(){
  ...
  try{webpush.setVapidDetails(VAPID_EMAIL,VAPID_PUBLIC_KEY,VAPID_PRIVATE_KEY);ready=true;return true;}catch{return false;}
}
...
async function unsubscribe(endpoint){await query(`DELETE FROM push_subscriptions WHERE subscription->>'endpoint'=$1`,[endpoint]);}
async function sendAll(title,body,url='/'){
  ...
  const results=await Promise.allSettled(subs.rows.map(r=>{
    ...
    return webpush.sendNotification(s,payload).catch(async e=>{if(e.statusCode===410||e.statusCode===404)await unsubscribe(s.endpoint).catch(()=>{});throw e;});
  }));
...
async function sendReportNotifications(reports){
  ...
  const lines=reports.map(r=>{const e={bullish:'▲',bearish:'▼',neutral:'↔'}[r.bias||'neutral']||'↔';return e+' '+r.symbol+': '+(r.candle_type||'').replace(/_/g,' ')+' · '+r.bias;});
